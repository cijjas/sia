...
from utils.metrics import mean_squared_error, mean_absolute_error, r2_score, median_absolute_error, custom_kfold
...
def main():
    ...
    n_splits = 5
    folds = custom_kfold(X_scaled, y_scaled, n_splits=n_splits, shuffle=True, random_state=42)
    results = []
    ...
    for train_index, test_index in folds.split(X_scaled):
        ...
        perceptron_linear.train(X_train, y_train, num_epochs)
        ...
        mape_linear = np.mean(np.abs((y_test_original - y_pred_linear) / y_test_original)) * 100
    ...
    linear_values = [avg_mse_linear, avg_mae_linear, avg_r2_linear, avg_rmse_linear, avg_mape_linear, avg_med_ae_linear]
    ...
    non_linear_normalized = [non_linear_values[i] / max_values[i] for i in range(len(metrics))]
    linear_normalized = [linear_values[i] / max_values[i] for i in range(len(metrics))]

    fig, ax = plt.subplots(figsize=(12, 6))

    rects1 = ax.bar(x - width/2, non_linear_normalized, width, label='Non-Linear Perceptron')
