...
font_small = pygame.font.Font(
    "../res/fonts/NeueHaasDisplayRoman.ttf", FONT_SIZE_SMALL
)  # Larger font for algorithm buttons and map name
font_large = pygame.font.Font(
    "../res/fonts/NeueHaasDisplayMediu.ttf", FONT_SIZE_LARGE
)  # Larger font for algorithm buttons and map name
font_larger = pygame.font.Font(
    "../res/fonts/NeueHaasDisplayMediu.ttf", 32
)  # Larger font for algorithm buttons and map name
# Confidence Colors
...
def prepare_mnist_data(path):
    ...
    training_data = [
        (x.reshape(784, 1), y.reshape(10, 1)) for x, y in zip(x_train, y_train)
    ]
...
def plot_probability_distribution(probabilities):
    ...
    custom_colors = [[c / 255 for c in PRIMARY]]  # Normalize BUTTON_COLOR
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            # Handle mouse clicks
                    ...
                    for button in buttons:
                        if button["rect"].collidepoint(event.pos):
                            if button == predict_button:
                                input_vector, processed_image = preprocess_grid(
                                    grid_array
                                )
                                ...
                                processed_image_surface = pygame.transform.scale(
                                    processed_image_surface,
                                    (processed_image_width, processed_image_height),
                                )

                                # Generate the probability distribution plot
                                prob_plot_surface = plot_probability_distribution(
                                    output.flatten()
                                )
                ...
                if (
                    mouse_buttons[0] or mouse_buttons[2]
                ):  # Left or right button is held down
                    mouse_pos = pygame.mouse.get_pos()
        ...
        for button in buttons:
            ...
            if button["rect"].collidepoint(mouse_pos):
                text_surface = button["font"].render(
                    button["text"], True, BUTTON_HOVER_COLOR
                )  # Change text color on hover
            else:
                text_surface = button["font"].render(
                    button["text"], True, BUTTON_COLOR
                )  # Default text color
        ...
        pygame.draw.line(
            screen,
            BOREDER_COLOR,
            (0, clear_button_rect.bottom + 20),
            (LEFT_WIDTH, clear_button_rect.bottom + 20),
            1,
        )
        ...
        if processed_image_surface:
            processed_image_rect = screen.blit(
                processed_image_surface, (processed_image_x, processed_image_y)
            )
            ...
            processed_image_label = font_small.render(
                "Input Image", True, PREDICTION_COLOR
            )
            label_padding = 5  # Small padding for the tag
            label_rect = processed_image_label.get_rect(
                topleft=(
                    processed_image_rect.left + label_padding,
                    processed_image_rect.top + label_padding,
                )
            )

            # Draw the label on the top-left corner as a small tag
            pygame.draw.rect(
                screen, BOREDER_COLOR, label_rect.inflate(10, 5)
            )  # Optional: draw background for the tag
        ...
        if prob_plot_surface:
            ...
            prediction_text_y = (
                processed_image_y + processed_image_height + 20
            )  # Fallback position
            ...
            for i, line in enumerate(lines):
                if line.startswith("Model is"):
                    ...
                    text_rect = text_surface.get_rect(
                        topright=(
                            right_align_x,
                            prediction_text_y + i * (FONT_SIZE_LARGE + 5),
                        )
                    )
                    screen.blit(text_surface, text_rect)
